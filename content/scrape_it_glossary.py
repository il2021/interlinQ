    ...
    def _getContentsInITGlossary(self):
        ...
        for c in self.consonant:
            for v in self.vowel:
                ...
                self.wordUrlList = [url.a.get('href') for url in self.soup.find(id='index-list').find_all('li')]
                self.wordUrlList = [self.domain + url[3:] for url in self.wordUrlList]  # 各50音に含まれる単語のurl
                # print(wordUrlList)
